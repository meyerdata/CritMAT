...
'''
Function to calculate the Herfindahl-Hirschman Index (HHI) based on production or trade data.
For advanced implementations using World Governance Index (WGI) and EU trade parameters, refer to hhi_wgi and hhi_wgi_tp functions.
...
'''
def hhi_basic(prod_data,timeframe=None,):
        ...
        for category in source_data['category'].unique():
            ...
            for material in category_data['material_name'].unique():
                ...
                if timeframe:
                    ...
                    prod_timeframe = material_data.groupby(['material_name','country_id','category']).agg({data_column: 'sum','source_name': lambda x: (x).unique()}).reset_index()
                    prod_timeframe['date_year'] = str(timeframe)
                    prod_timeframe[data_column + '_total'] = prod_timeframe[data_column].sum()
                else:
                    #The folowing groupby is used for trade data, where multiple sources may need to be combined
                    prod_timeframe = material_data.groupby(['material_name','country_id','category','date_year']).agg({data_column: 'sum','source_name': lambda x: (x).unique()}).reset_index()
                ...
                prod_timeframe['hhi'] = (prod_timeframe[data_column]/prod_timeframe[data_column + '_total']*100)**2
