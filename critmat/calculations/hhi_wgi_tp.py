...
'''
Function to calculate the Herfindahl-Hirschman Index (HHI) with World Governance Index (WGI) and EU trade parameters weighting.
...
'''
def hhi_wgi_tp(prod_data,wgi_data,tradeparameters,timeframe=[2016,2017,2018,2019,2020]):
    ...
    #Tradeparameters are currently constant over time, but in the future they should also be filtered by timeframe
    ...
    #By default the world scope is assumed for the trade parameters, but if the data is trade data, the scope is set to EU specific.
    scope = 'world' if data_column == 'quantity' else 'eu'
    ...
    for source in prod_data['source_name'].unique():
        ...
        for category in source_data['category'].unique():
            ...
            for material in category_data['material_name'].unique():
                ...
                #Currently trade parameters are constant over time, but in case they change in the future, we merge them here, later they should be merged by date_year as well.               
                material_data = material_data.merge(tradeparameters[tradeparameters['scope'] == scope], on=['country_id','material_name','category'], how='left', suffixes=('', '_tp'))  

                #If the category cannot be matched the trade parameter will be NaN, in this case we skip the material to avoid errors in the HHI calculation.
                if material_data['parameter'].isnull().all():
                    continue
                ...
                if timeframe:
                    ...
                    prod_timeframe = material_data.groupby(['material_name','country_id','category']).agg({data_column: 'sum', 'mean': 'mean','parameter': 'mean','source_name': lambda x: (x).unique()}).reset_index()
                    prod_timeframe['date_year'] = str(timeframe)
                    prod_timeframe[data_column + '_total'] = prod_timeframe[data_column].sum()
                else:
                    #The folowing groupby is used for trade data, where multiple sources may need to be combined
                    prod_timeframe = material_data.groupby(['material_name','country_id','category','date_year']).agg({data_column: 'sum', 'mean': 'mean','parameter': 'mean','source_name': lambda x: (x).unique()}).reset_index()
                ...
                prod_timeframe['hhi'] = (prod_timeframe[data_column]/prod_timeframe[data_column + '_total']*100)**2*prod_timeframe['mean']*prod_timeframe['parameter']
    ...
    hhi['hhi'] = hhi['hhi'].round(0) #we round the HHI to 0 decimal places
    return hhi