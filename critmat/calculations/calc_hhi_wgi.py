...
def calc_hhi_wgi(prod_data,wgi_data,timeframe=range(2016,2020)):
    ...
    for source in prod_data['source_id'].unique():
        ...
        for category in source_data['category'].unique():
            ...
            for material in category_data['material_id'].unique():
                ...
                prod_year = prod_year.merge(wgi_data, on=['country_id'], how='left', suffixes=('', '_wgi'))
                prod_year['hhi'] = (prod_year['quantity']/total*100)**2*(prod_year['mean']-2.5)*-2
...
if __name__ == "__main__":
    ...
    hhi = calc_hhi_wgi(prod_data, wgi_data)
    print(hhi)