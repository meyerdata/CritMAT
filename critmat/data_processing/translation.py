...
def standard_country_name(ser,problemcountries,ccodes=False,cnames=False):
    ...
    countries = {'United States': [r'^(?!.*islands).*united.?states|^u\\.?s\\.?a\\.?$|^u\\.?s\\.?$','vereinigte staaten','amerika'], 
        ...
        'Congo Republic': [r'^(?!.*\\bdem)(?!.*\\bdr)(?!.*kinshasa)(?!.*zaire)(?!.*belg)(?!.*l\\w{1,2}opoldville)(?!.*free)(^rep.*).*\\bcongo.*(?!.*\\bdem)(?!.*\\bdr).*|\\bwest.*congo|^congo[,;\\s]*(?!.*dem)rep.*?$|^congo$|\\bcongo.*brazza.*',r'Congo Rep\S*'], 
        ...
        'DR Congo': [r'\\bdem.*congo|congo.*\\bdem|congo.*\\bdr|\\bdr.*congo|\\bd\\.?r\\.?c|\\bd\\.?r\\.?o\\.?c|\\br\\.?d\\.?c|belgian.?congo|congo.?free.?state|kinshasa|zaire|l\\w{1,2}opoldville|^the\\ congo$|^RDC$|^DROC$|\\bcongo.*dem.*','Congo Kinshasa', r'(DR\s*Congo|Congo\s*DR)', 'DR Kongo', 'kongo dem'], 
        ...
        'South Korea': [r'^(?!.*dem)(?!.*peo)(?!.*nor)(?!.*n)(?!.*dpr)(?!d\\.p\\.r).*\\bkorea|\\br\\.?o\\.?k\\b',  r'(South\s*Korea|Korea\s*South)', 'Korea Republic',r's(ü|ue)dkorea'], 
